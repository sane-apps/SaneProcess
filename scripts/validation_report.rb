...
class ValidationReport
  ...
  AGENTMEMORY_STATUS_TIMEOUT_SECONDS = 2.0
  AGENTMEMORY_STATUS_TERMINATION_GRACE_SECONDS = 0.25
  ...
  def agentmemory_status_output
    ...
    capture_agentmemory_status(command: command)[:output]
  end

  def capture_agentmemory_status(command:, timeout_seconds: AGENTMEMORY_STATUS_TIMEOUT_SECONDS)
    ...
    begin
      ...
      stdin.close
      readers << [Thread.new { drain_process_stream(stdout, stdout_buffer) }, stdout]
      readers << [Thread.new { drain_process_stream(stderr, stderr_buffer) }, stderr]
      ...
      terminate_process_group(wait_thr) if wait_thr&.alive?
      stdin.close if stdin && !stdin.closed?
      readers.each { |thread, stream| finish_process_reader(thread, stream) }
    ...
    return agentmemory_capture_receipt('', false, 125, duration_seconds) if capture_failed
    return agentmemory_capture_receipt('', true, 124, duration_seconds) if timed_out

    stdout_buffer.force_encoding(Encoding.default_external)
    stderr_buffer.force_encoding(Encoding.default_external)
    status = wait_thr.value
    exit_code = status.exitstatus || (status.termsig ? 128 + status.termsig : 1)
    output = status.success? ? stdout_buffer : "#{stdout_buffer}\n#{stderr_buffer}"
    agentmemory_capture_receipt(output, false, exit_code, duration_seconds)
  rescue StandardError
    duration_seconds = Process.clock_gettime(Process::CLOCK_MONOTONIC) - started_at
    agentmemory_capture_receipt('', false, 125, duration_seconds)
  end

  def drain_process_stream(stream, buffer)
    loop { buffer << stream.readpartial(4096) }
  rescue EOFError, IOError
    nil
  end

  def finish_process_reader(thread, stream)
    return if thread.join(AGENTMEMORY_STATUS_TERMINATION_GRACE_SECONDS)

    stream.close unless stream.closed?
    return if thread.join(AGENTMEMORY_STATUS_TERMINATION_GRACE_SECONDS)

    thread.kill
    thread.join(AGENTMEMORY_STATUS_TERMINATION_GRACE_SECONDS)
  rescue IOError
    nil
  end
  ...
  def terminate_process_group(wait_thr)
    ...
    signal_process_group('TERM', wait_thr.pid)
    return if wait_thr.join(AGENTMEMORY_STATUS_TERMINATION_GRACE_SECONDS)

    signal_process_group('KILL', wait_thr.pid)
    wait_thr.join(AGENTMEMORY_STATUS_TERMINATION_GRACE_SECONDS)
  end
  ...
  def agentmemory_capture_receipt(output, timed_out, exit_code, duration_seconds)
    {
      output: output,
      timed_out: timed_out,
      exit_code: exit_code,
