...
# 7. Uploads DMG to Cloudflare R2
...
# Step 7: Create GitHub Release (without DMG - DMGs go to R2)
...
# Step 8: Read release metadata and update appcast
...
# Read signature from .meta file generated by release.sh
META_FILE="build/$PROJECT_NAME-$VERSION.meta"
if [ -f "$META_FILE" ]; then
    ED_SIG=$(grep "^SIGNATURE=" "$META_FILE" | cut -d= -f2-)
    BUILD_NUM=$(grep "^BUILD=" "$META_FILE" | cut -d= -f2-)
else
    log_warn "Meta file not found at $META_FILE - signature extraction may fail"
    ED_SIG="SIGNATURE_NEEDED"
    BUILD_NUM="$PROJECT_VERSION"
fi

if [ "$ED_SIG" = "UNSIGNED" ] || [ -z "$ED_SIG" ]; then
    log_error "No valid EdDSA signature found. Appcast will need manual update."
    ED_SIG="SIGNATURE_NEEDED"
fi
...
NEW_ITEM=$(cat <<ITEMEOF
        <item>
            ...
            <enclosure url="https://dist.${LOWER_PROJECT}.com/updates/${PROJECT_NAME}-${VERSION}.dmg"
...
# Write NEW_ITEM to a temp file to avoid sed quoting issues
echo "$NEW_ITEM" > /tmp/${LOWER_PROJECT}_new_item.xml
sed -i '' "/<title>$PROJECT_NAME Changelog<\/title>/r /tmp/${LOWER_PROJECT}_new_item.xml" docs/appcast.xml
rm /tmp/${LOWER_PROJECT}_new_item.xml
...
log_warn "Run the following to upload DMG:"
echo "  npx wrangler r2 object put ${LOWER_PROJECT}-downloads/updates/${PROJECT_NAME}-${VERSION}.dmg --file=\"${DMG_PATH}\""
echo ""
...
echo "  - R2: https://dist.${LOWER_PROJECT}.com/updates/${PROJECT_NAME}-${VERSION}.dmg"
echo "  - Appcast: Updated in docs/appcast.xml"
echo ""
echo -e "${YELLOW}Manual steps remaining:${NC}"
echo "  1. Upload DMG to R2 (see command above)"
echo "  2. Deploy website: npx wrangler pages deploy ./docs --project-name=${LOWER_PROJECT}-site"
echo "  3. Reply to any GitHub issues fixed in this release"
echo ""
