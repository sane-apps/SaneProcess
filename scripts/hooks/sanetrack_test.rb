...
require_relative 'sanetrack_test_mcp_verification'
require_relative 'sanetrack_test_runner_proofs'
...
module SaneTrackTest
  def self.run(process_result_proc, detect_actual_failure_proc, normalize_error_proc,
    ...
    Dir.mktmpdir('sanetrack-bash-mutation-') do |dir|
      ...
      StateManager.reset(:edits)
      File.write('PreExisting.swift', "dirty before session\n")
      preexisting_path = File.realpath(File.expand_path('PreExisting.swift', dir))
      StateManager.update(:edits) do |edits|
        edits[:baseline_dirty_files] = [preexisting_path]
        edits
      end
      ...
      if edits[:count].to_i.positive? && tracked_paths.include?(changed_path) && !tracked_paths.include?(preexisting_path)
        passed += 1
        warn '  PASS: Bash mutation tracking records only post-session git changes'
      else
        failed += 1
    ...
    # Native review credits are intentionally source-bound. The isolated hook
    # self-test project must therefore have a real committed Git snapshot.
    _git_root, git_status = Open3.capture2e('git', 'rev-parse', '--show-toplevel')
    unless git_status.success?
      system('git', 'init', '-q')
      system('git', 'config', 'user.email', 'test@example.com')
      system('git', 'config', 'user.name', 'Test')
      system('git', 'add', '.saneprocess')
      system('git', 'commit', '-q', '-m', 'self-test source')
    end
    ...
    runner_passed, runner_failed = SaneTrackRunnerProofTest.run(
      process_result_proc,
      method(:write_runner_proof_fixture),
      method(:ship_clearance_proof)
    )
    passed += runner_passed
    failed += runner_failed

    mcp_passed, mcp_failed = SaneTrackMcpVerificationTest.run(process_result_proc, invalidate_empty_research_proc)
    passed += mcp_passed
    failed += mcp_failed
