...
Dir.mktmpdir('release-guard-app-') do |project_dir|
  ...
  # A recent unrelated verify receipt is not exact TestFlight package proof.
  ...
  begin
    ...
    t('Release guard blocks TestFlight artifact altool upload with only verify proof', altool_tf_status.exitstatus == 2)
    t('TestFlight block requires exact adjacent proof', altool_tf_err.include?('exact adjacent mode-0600 proof'))
    ...
    t('Release guard resolves but blocks $ART path without exact proof', altool_var_status.exitstatus == 2)
    ...
    t('Release guard resolves but blocks inline ART path without exact proof', altool_inline_status.exitstatus == 2)
  ensure
    FileUtils.rm_rf(verify_fixture)
  end

  # Isolate "no proof" in a separate .saneprocess tree.
  ...
  begin
    ...
    t('Release guard blocks TF altool without exact artifact proof', altool_stale_status.exitstatus == 2)
    t('TF-without-proof block points at canonical proof producer', altool_stale_err.include?('SaneProcess/scripts/testflight_artifact_proof.rb'))
  ensure
