...
REQUIRED_GUARD_FILES="sane_action_guard.rb core/action_authorization.rb"
    ...
    "PreToolUse": [
      ...
      },
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "ruby \"$CLAUDE_PROJECT_DIR\"/scripts/hooks/sane_action_guard.rb",
            "timeout": 30000
          }
        ]
      },
...
if [ "$INSTALL_CLAUDE" -eq 1 ]; then
    ...
    for guard in $REQUIRED_GUARD_FILES; do
        if [ ! -f "scripts/hooks/$guard" ]; then
            echo -e "   ${RED}x${NC} scripts/hooks/$guard missing"
            ERRORS=$((ERRORS + 1))
        elif ! "$RUBY_CMD" -c "scripts/hooks/$guard" >/dev/null 2>&1; then
            echo -e "   ${RED}x${NC} scripts/hooks/$guard has syntax errors"
            ERRORS=$((ERRORS + 1))
        fi
    done
