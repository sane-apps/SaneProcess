...
class DirectAppFunnelReportTests(unittest.TestCase):
    ...
    def test_donation_redirects_are_separate_from_checkout(self):
        row = {"app": "sanebar", "event": "website_donation_redirected",
               "date": "2026-07-19", "count": 2, "platform": "web", "channel": "website"}
        output = self.render_main({"events": [row], "event_dimensions": [row]}, "--events", "--days", "7")
        line = next(line for line in output.splitlines() if line.startswith("website_donation_redirected"))
        self.assertEqual(line.split(), ["website_donation_redirected", "2", "2", "2"])
        self.assertFalse(any(line.startswith("checkout_clicked") for line in output.splitlines()))
