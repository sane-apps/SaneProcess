...
module SaneCwsReviewWatch
  ...
  def credential_configuration(process_env: ENV, env_path: DEFAULT_ENV_PATH)
    ...
    ENV_NAMES.each_with_object({}) do |name, result|
      result[name] = process_env.key?(name) ? process_env[name].to_s : cached[name].to_s
    end
  end
  ...
  def credential_mode(configuration)
    refresh_names = %w[SANE_CWS_CLIENT_ID SANE_CWS_CLIENT_SECRET SANE_CWS_REFRESH_TOKEN]
