...
module SaneMasterModules
  ...
  module UpgradePathProof
    ...
    def upgrade_path_proof(args = [])
      ...
      package_path = config['package_path'].to_s.strip
    ...
    def upgrade_path_validate_monitor_receipt!(path, project_root:, scheme:, package_path: nil, test_plan: nil, destination: nil, test_selector:, started_at:, finished_at:,
      ...
      if destination
        ...
        destination_index = command.is_a?(Array) ? command.index('-destination') : nil
        unless destination_index && command[destination_index + 1] == destination
          raise 'Canonical monitor_tests receipt has invalid destination.'
        end
      end
