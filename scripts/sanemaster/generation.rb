...
module SaneMasterModules
  ...
  module Generation
    ...
    def generate_test_file(args)
      ...
      if options[:type] == 'ui'
        puts '⚠️  UI tests not yet implemented (__PROJECT_NAME__UITests directory does not exist)'
        puts '   Creating unit test instead...'
        options[:type] = 'unit'
      end

      test_dir = '__PROJECT_NAME__Tests'
    ...
    def check_xcodegen(files)
      ...
      project_path = File.expand_path('__PROJECT_NAME__.xcodeproj', Dir.pwd)
    ...
    def generate_xctest_content(test_name, options)
      ...
      <<~SWIFT
        //
        //  #{test_name}.swift
        //  #{options[:type] == 'ui' ? '__PROJECT_NAME__UITests' : '__PROJECT_NAME__Tests'}
        //
    ...
    def collect_project_files(project_path)
      ...
      project.files.each do |file|
        ...
        project_files.add(path.sub(%r{^__PROJECT_NAME__/}, ''))
        project_files.add("__PROJECT_NAME__/#{path}") unless path.start_with?('__PROJECT_NAME__/')
      end
    ...
    def find_missing_files(files, project_files)
      ...
      files.each do |file|
        ...
        normalized = file.start_with?('__PROJECT_NAME__/') ? file : "__PROJECT_NAME__/#{file}"
        path_without_prefix = file.sub(%r{^__PROJECT_NAME__/}, '')
