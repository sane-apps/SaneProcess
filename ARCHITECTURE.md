...
Portfolio validation is a read-only observer of this lifecycle, never a
service owner. Its `agentmemory status` child runs in a dedicated process group
with a two-second deadline while stdout and stderr are drained concurrently.
On timeout, validation sends TERM to the process group, escalates to KILL after
a bounded grace period, and records `timed_out`, `exit_code`, and
`duration_seconds` before continuing the report. Validation may report the
runtime unavailable, but it must never start or retry AgentMemory and must not
hang the broader portfolio verdict.
